...
function parseDate(dateStr, timezone = "Europe/Dublin") {
  ...
  if (input === "today") {
    ...
    date = new Date(today);
    date.setDate(date.getDate() - 1);
  } else if (input.startsWith("next ") || input.startsWith("this ")) {
    // "next monday", "this friday", "next week", etc.
    const part = input.replace(/^(next|this) /, "");
    const dayMap = {
  ...
  } else if (/^(sunday|monday|tuesday|wednesday|thursday|friday|saturday)$/.test(input)) {
    // Just a day name like "friday" - find the next occurrence
    const dayMap = {
      sunday: 0, monday: 1, tuesday: 2, wednesday: 3,
      thursday: 4, friday: 5, saturday: 6
    };
